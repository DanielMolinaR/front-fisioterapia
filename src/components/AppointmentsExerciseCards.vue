<template>
  <v-container fluid>
    <v-col cols="12" xl="2">
      <v-row class="pb-6 mt-4" align="center" justify="center">
        <v-btn-toggle v-model="toggle_exclusive" rounded>
          <v-btn value="appointments" @click="changeToAppointments"
            >Mis citas</v-btn
          >
          <v-btn value="exercises" @click="changeToExercises"
            >Mis ejercicios</v-btn
          >
        </v-btn-toggle>
      </v-row>
    </v-col>
    <v-row
      align="center"
      justify="center"
      v-if="this.condition === 'exercises' && user_level == 0"
    >
      <v-col
        v-for="card in this.Ecards"
        :key="card.title"
        :cols="12"
        sm="6"
        md="6"
    ...
    <v-row
      align="center"
      justify="center"
      v-if="this.condition === 'appointments'"
    >
      <v-col
        v-for="card in this.Acards"
        :key="card.date"
        :cols="12"
        sm="6"
        md="6"
